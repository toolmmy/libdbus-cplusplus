 ...
 *
 * Unix file descriptor added to the library which is used to forward data
 * from a dbus service to the client.
 *
 * The UnixFd class is using simple file descriptors managed in the class.
 * Each file descriptor gets closed within the destructor. It is also possible
 * to copy a UnixFd into another. In this case it is important to take the
 * ownership of the source UnixFd in order to avoid the closing of the fd.
 */
struct DXXAPI UnixFd
{
	...
	/**
	 * Returns true if the internal file descriptor is open. Otherwise this method returns false.
	 */
	bool isOpen() const;
...
template<typename T1, typename T2 = Invalid, typename T3 = Invalid, typename T4 = Invalid, typename T5 = Invalid,
		typename T6 = Invalid, typename T7 = Invalid, typename T8 = Invalid, typename T9 = Invalid,
		typename T10 = Invalid, typename T11 = Invalid, typename T12 = Invalid, typename T13 = Invalid,
		typename T14 = Invalid, typename T15 = Invalid, typename T16 = Invalid // nobody needs more than 16
>
struct Struct
{
	T1 _1;
...
{
	iter.append_unix_fd(val.getFileDescriptor());
	return iter;
}
